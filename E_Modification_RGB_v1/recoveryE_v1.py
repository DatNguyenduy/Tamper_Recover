from detectE_v1 import *
from utilsE_v1 import create_luck_up_table
import numpy as np
def __init__():
    pass
def packbit1(list_10_bits):
    joint_8b = np.concatenate((list_10_bits[6:8],list_10_bits[2:6],np.array([0,0],dtype=np.uint8)),dtype=np.uint8)
...
def stage1_1block2_recovery(valid_block2):
    joint_10bits = wm_au_extract(block2=valid_block2)
    r_block2 = np.full((2,2),packbit(joint_10bits[:6]),dtype=np.uint8)
    r_block2[1,1] = packbit1(joint_10bits)
    return r_block2
    ...
    for channel in range(3):
        ...
        for i in range(len(list_invalid[0])):
            ...
            if detect_matrix[ir_co,ic_co]:
                ...
                # lv4_matrix[ir*2+1,ic*2+1,channel] = 1
