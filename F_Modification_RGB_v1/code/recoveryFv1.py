...
def stage1_1block2_recovery(valid_block2,ir_co,ic_co,channel):
    joint_8bits = wm_au_extract(block2=valid_block2)
    joint_8bits[0] = joint_8bits[0]^add_XOR_all_string(format((channel+1)*ir_co+ic_co,'b'))
    joint_8bits[1] = joint_8bits[1]^add_XOR_all_string(format(np.abs((channel+1)*ir_co-ic_co),'b'))
    ...
    for channel in range(3):
        ...
        for i in range(len(list_invalid[0])):
            ...
            if detect_matrix[ir_co,ic_co]:
                ...
                r_block2= stage1_1block2_recovery(valid_block2=valid_block2,ir_co=ir_co,ic_co=ic_co,channel=channel)
